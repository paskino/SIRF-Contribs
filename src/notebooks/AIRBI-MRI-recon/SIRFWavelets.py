...
def _take_abs_input(func):
    ...
    def _take_abs_decorator(self, x: DataContainer, *args, **kwargs):
        ...
        # r = rgeo.allocate(0)
        # TODO this should allocate a real image of the same shape of the input x
        # In SIRF with Gadgetron ImageData this is not possible as these are 
        # only containing complex data.
        r = x * 0
...
def _abs_and_project(func):
    ...
    def _abs_project_decorator(self, x: DataContainer, *args, **kwargs):
        ...
        r = x * 0
        r.fill(np.abs(x.as_array()).astype(real_dtype))
        Phi = np.exp((1j*np.angle(x.as_array())))
...
class SIRFWaveletOperator(WaveletOperator):
    def __init__(self,
                 domain_geometry,
                 level=None,
                 wname="haar",
                 **kwargs):
        # needs to define domain_geometry,
        # Optionally range_geometry=None
        shape = domain_geometry.shape

        ig = ImageGeometry(
            voxel_num_x = shape[2] if len(shape) > 2 else 1,
            voxel_num_y = shape[1],
            voxel_num_z = shape[0]
        )
        super().__init__(domain_geometry=ig, level=level, wname=wname, **kwargs)